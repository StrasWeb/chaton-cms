...
$dom->html->body->div->div->addElement("h2", _("Custom footer"), array("class"=>"subtitle"));
if(isset($_GET["lang"])){
...
if(isset($_POST['modified'])) {
	...
	if($plugin->setParam("footer_default", $plugin->default, $lang) && $plugin->setParam("footer_content", $plugin->content, $lang)){	
...
$dom->html->body->div->div->addElement("form",null, array("action"=>"index.php?tab=plugin&dir=".$plugin->dir, "method"=>"post"));
		$dom->html->body->div->div->form->addElement("input", null, array("type"=>"checkbox", "id"=>"default", "name"=>"default"));
		if($plugin->default){
			$dom->html->body->div->div->form->input->setAttribute("checked", true);
		}
		$dom->html->body->div->div->form->addElement("label", _("Show default footer"), array("for"=>"default"));
